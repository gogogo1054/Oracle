...
설명 :  레코드를 조작할 때 사용하는 쿼리문. 앞에서 학습했던 select문을
비롯하여 update(레코드수정), delete(레코드삭제), insert(레코드입력)가 있다.
****************/
-- study 계정

/*
레코드 입력하기 : insert
    레코드 입력을 위한 쿼리문으로 문자형은 반드시 ''로 감싸줘야 한다.
...
외부에서는 변경된 레코드를 확인할 수 없다. 여기서 말하는 외부란 Java/Jsp와
같은 오라클 이외의 프로그램을 말한다.
    *트랜잭션이란 송금과 같은 하나의 단위작업을 말한다.
*/
commit; -- 되도록 커밋하기전에 모든 검토를 하고 마지막에 해주는 것이 좋음

-- 커밋 이후 새로운 레코드를 삽입하면 임시테이블에 저장된다.
insert into tb_sample values (60, '금융팀', '광주', '아이린');
-- 오라클에서 확인하면 실제 삽입된 것처럼 보인다. 하지만 실제 반영되지 않은
-- 상태이다.
select * from tb_sample;
...
/*
레코드 수정하기 : update
형식] update 테이블명 set 컬럼=값1, 컬럼2=값2 ... where 조건;
    * 조건이 없는 경우 모든 레코드가 한꺼번에 수정된다.
    * 테이블명 앞에 from이 들어가지 않는다.
*/
-- 부서번호가 40인 레코드의 지역을 미국으로 수정하시오.
...
/*
DML문 : 테이블을 입력 및 조작하는 쿼리문 - 자주 사용
(Data Manipulation Language : 데이터 정의어)
...
/*
문제 1
DDL문 연습문제 2번에서 만든 "pr_emp" 테이블에 다음과 같이 레코드를 삽입하시오.
...
/*
문제 2
pr_emp 테이블의 eno가 짝수인 레코드를 찾아서 job컬럼의 내용을 다음과 같이
변경하시오
job=job||'(짝수)'
job=concat(job, '(홀수)')
*/
...
select * from pr_emp where mod(eno, 2)=1;
