...
* 이와같이 제약조건이나 뷰, 프로시저 등의 정보를 저장하고 있는 시스템 테이블을
"데이터 사전"이라고 한다.
*/
select * from tab;
...
create table 테이블명 (
컬럼명 자료형(크기)
[constaints 제약명] primary key (컬럼명)
*/
...
/*
형식3] 테이블을 생성한 후 alter문으로 제역전문 추가
...
/*
테이블을 생성한 후 alter명령을 통해 제약조건을 보여할 수 있다.
제약명의 경우 생략이 가능하다.
*/
...
-- PK는 테이블당하나만 생성할 수 있다. 따라서 해당 문장은 에라가 발생한다
...
insert into tb_unique values(6, '황정민', '010-6666-6666', '영화배우'); -- 입력
insert into tb_unique values(7, '이순재', '010-6666-6666', '영화배우'); -- 에러
/*
(telephone, nickname)은 동일한 제약명으로 설정되었으므로 두개의 컬럼이 동시에
동일한 값을 가지는 경우가 아니라면 중복된 값이 허용된다. 즉 5번과 6번은 서로
다른 데이터로 인식하므로 입력되고, 6번과 7번은 동일한 데이터로 인식되어 입력되지
않는다.
*/
select*from user_cons_columns;
...
/*
Foreign key : 왜래키, 참조키
왜래키는 참조 무결성을 유지하기 위한 제약조건으로 만약 테이블간에 외래키가 
설정되어 있다면 자식테이블에 참조값이 존재할 경우 부모테이블의 레코드는 
삭제할 수 없다.
...
f_name varchar2(50),
/*
자식테이블인 tb_foreign에서 부모테이블인 tb_primary2의 user_id 컬럼을 참조하는
외래키를 생성한다.
*/
...
-- 오류 발생 부모테이블에는 gildong이라는 아이디가 없기 때문.
insert into tb_foreign1 values(1, '홍길동', 'gildong');
-- 입력 성공. 부모테이블에 white라는 아이디가 존재하기 때문.
...
삭제해야 한다.
*/
-- 오류발생(자식테이블의 레코드를 먼저 삭제한 후 삭제해야함)
...
delete from tb_primary2 where user_id = 'white'; -- 오류 없이 삭제
...
/*
두 개의 테이블이 외래키(참조키)가 설정되어 있는 경우 부모테이블의 참조할
레코드가 없으면 자식테이블에 insert 할 수 없다. 자식테이블의 부모를 참조하는
레코드가 남아있으면 부모테이블의 레코드를 delete 할 수 없다.
*/
...
f_name varchar2(30),
f_date date, foreign key (f_id) 
references tb_primary1(idx)
);
select * from user_cons_columns;
/*
...
on delete cascade -- 부모가 삭제되면 같이 삭제된다는 옵션
);
/*
외래키가 설정된 경우 반드시 부모테이블에 레코드를 먼저 입력한 후 자식 테이블에
입력해야 한다.
*/

-- 부모테이블에 레코드 입력
insert into tb_primary4 values('student', '훈련생');
-- 자식테이블에 레코드 입력
...
on delete set null -- 부모가 삭제되면 null값을 넣는다는 옵션 ( 연결된 레코드만 )
);

-- 부모테이블에 레코드 입력
insert into tb_primary5 values('student', '훈련생');
-- 자식테이블에 레코드 입력
...
/*
on delete set null 옵션으로 자식테이블의 레코드는 삭제되지않고 참조키 부분만
null값으로 변경된다. 따라서 더이상 참조할 수 없는 레코드로 변경된다. (join 불가)
*/
...
m_idx number(10) primary key, -- pk 이므로 not null
m_id varchar2(30) not null, -- nn ( 공백이 있으면 안되며, 중요하다는 뜻 )
m_pw varchar2(30) null, -- null 허용
m_name varchar2(40) -- default 값으로 null 허용(이와같이 사용)
);
...
/*
default : insert시 아무런 값도 입력하지 않았을 때 자동으로 삽입되는 데이터를
지정할 수 있다.
*/
...
insert into tb_default values ('aaaa', '1234'); -- 1234
insert into tb_default values ('bbbb'); -- 컬럼자체가 없으므로 오류발생
...
insert into tb_default values ('ffff', default); -- default값(qwer) 입력

-- 아이디만 입력하겠다 지정하면 pw는 자동으로 default(qwer) 입력
insert into tb_default (id) values ('gggg'); 


/*
default 값을 입력하려면 insert문에서 컬럼 자체를 제외시키거나 default 키워드를
사용해야 한다.
*/
...
/*
check : Domain(자료형) 무결성을 유지하기 위한 제약조건으로 해당 컬럼에 잘못된
데이터가 입력되지 않도록 유지하는 제약조건이다.
*/
--M, F만 입력을 허용하는 제약조건
...
insert into tb_check2 values (11); -- 제약조건 위배로 입력실패
