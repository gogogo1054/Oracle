...
파일명 :  Or15DCL.sql
DC: " Data Control language(데이터 제어언어) - 사용자 권한
...
*/
-- 오라클 12C 이후부터는 해당 명령을 먼저 실행한 후 계정을 생성해야한다.
...
/*
계정 생성 직후 cmd에서 sqlplus 명령으로 접속을 시도해보면 login denied 에러가
발생된다. create session 권한 즉 접속권한이 없어 접속할 수 없다.
*/
...
grant 시스템권한1, 권한2, ...N
    to 사용자계정명
        [with grant옵션]
*/
-- 접속 권한을 부여한다. 권한을 부여할때는 to를 부여한다.
...
/*
quit 혹은 exit명령으로 접속을 해제한 후 다시 접속하면 기존 암호로는 접속되지
않는다. 변경한 비밀번호인 4321로 접속해야 한다.
*/
...
묶어놓은 것을 말한다.
* 실습을 위해 새롭게 생성한 계정에 connect resource롤을 주로 부여한다.
*/
alter session set "_ORACLE_SCRIPT"=true;
create user test_user2 identified by 1234;

grant connect, resource to test_user2; -- 접속권한, 생성권한

/*
test_user2 계정에 롤을 통해 권한을 부여한 후 접속 및 테이블 생성이 정상적으로
된다.
*/
...
-- test_user3에 접속 및 테이블 생성, 뷰 생성 권한이 부여됨.
...
/*
test_user3은 my_role을 통해 권한을 부여받았으므로 해당 롤을 삭제하면 부여받았던
모든 권한이 회수(revoke)된다.
즉, 롤 삭제 후에는 접속이 불가능하다.
*/
...
-- test user1으로는 접속할 수 없게된다. -- 접속 시 오류발생
...
select * from dba_users where lower(username) = 'test_user2'; -- 검색 됨
select * from dba_users where username = upper('test_user3'); -- 검색 됨
...
/*
테이블 스페이스란?
디스크 공간을 소비하는 테이블과 뷰, 그리고 그 밖의 다른 데이터베이스 객체들이
저장되는 장소이다. 예를 들어 오라클을 최초로 설치하면 hr계정의 데이터를 
저장하는 user라는 테이블 스페이스가 자동으로 생성된다.
*/
-- 테이블 스페이스 조회하기
...
-- 앞에서 생성한 test_user2 사용자의 테이블 스페이스 확인하기
...
-- 앞에서 지정한데로 테이블 스페이스 users를 사용하고 있다.
select username, default_tablespace from dba_users
...
/*
이미 생성된 사용자의 디폴트 테이블 스페이스를 변경하고 싶을떄는
alter user 명령을 사용한다.
*/
