...
/*
PL/SQL문장의 끝에는 /를 붙여야 하는데, 만약 없으면 호스트 환경으로
빠져나오지 못한다. 즉 PL/SQL 문장을 위해 필요하다. 
...
-- 예제3] 참조변수1 ( 하나의 컬럼 참조 )
/*
참조변수 : 특정 데이터의 컬럼을 참조하는 변수로써 동일한 자료형과 크기로
선언하고 싶을때 사용한다.
형식] 테이블명.컬럼명%type
=> 테이블의 '하나'의 컬럼만 참조한다.
*/
...
대입 후 화면상에 출력하는 PL/SQL문을 작성하시오.
단, 변수는 기존테이블의 자료형을 참조하는 '참조변수'로 선언하시오.
*/
-- 시나리오의 조건에 맞는 select문
select employee_id, salary, department_id from employees
where department_id = 10;

declare
-- 사원테이블의 특정 칼럼의 타입과 크기를 그대로 참조하는 변수로 선언한다.
...
declare
/*
사원테이블 전체 컬럼을 참조하는 참조변수로 선언한다. 이때 테이블명 뒤에
%rowtype을 붙여 선언한다.
*/
...
*/
    select *
        into emp_row 
        -- 위 문구를 주석처리하고 from문구까지만 묶어 실행 가능 ( 출력확인 )
...
/*
복합변수
class를 정의하듯 필요한 자료형을 묶어 하나의 자료형을 만든 후 생성하는
변수를 말한다.
형식] type 복합변수자료형 is record(
        컬럼명1 자료형(크기),
        컬럼명2 테이블명.컬럼명%type
        );
앞에서 선언한 자료형을 기반으로 변수를 생성한다.
복합변수 자료형을 만들어내는 일반변수와 참조변수 2가지를 복합해서 
사용할 수 있다.
*/
...
-- 3개의 값을 저장할 수 있는 복합변수자료형을 저장한다.
    ...
    */
    record3 emp_3type;
begin
    -- select절에서 가져온 3개의 값을 복합변수에 저장한다.
    select employee_id, first_name||' '||last_name, job_id
        into record3
    from employees where employee_id = 100;
...
위에서 생성한 자료형을 이용하여 복합변수 rec2를 생성 후 사원번호 100번의
정보를 할당한다.

2. 1의 내용을 출력한다.

3. 위 내용을 완료한 후 치환연산자를 사용하여 사원번호를 사용자로부터 
...
from employees where employee_id = 100; -- nvl 추가 - 답안

set serveroutput on; -- sqld 재실행시 꼭 실행해주어야 아래 문구들이 출력됨

declare
    -- 4개의 멤버를 가진 복합변수 자료형 선언
    type empTypes is record(
        emp_id employees.employee_id%type,
        -- 값의 변경이 없는 경우라면 참조변수로 선언하는 것이 좋다.
        emp_name varchar2(50),
...
begin
    ...
    nvl(commission_pct,0) -- nvl 추가 - 추가시 보너스율(0) 출력됨
    into rec2
    from employees where employee_id = 100;
...
-- 만약 출력이안된다면 cmd에서 확인해보면 정상적으로 출력된다.
print return_var;
...
declare
    ...
    sumNum number := 0;
    -- 변수명으로 sum을 사용할 수 없다. 
    -- 예약어(그룹함수)이므로 오류가 발생하기 떄문이다.
begin
    loop
        -- 증가하는 변수 i를 누적해서 sumNum에 대입후 더한다.
        sumNum := sumNum + i;
        -- 변수 i는 1씩 증가한다.
        i := i + 1;
        -- 10을 초과하면 탈출한다.
...
선언방법]
    Cursor 커서명 Is  
        select 쿼리문 ( 단, into절이 없는 형태로 기술해야한다.)
        
    Open Cursor : 쿼리를 수행하라는 의미. 즉 open할때 Cursor선언시의 select
    문장이 실행되어 결과셋을 얻게된다. Cursor는 그 결과셋의 첫번째행에
    위치하게 된다.
    
    Fetch~ Into~ : 결과셋에 하나의 행을 읽어들이는 작업으로 결과셋의 인출(Fetch)
    후에 Cursor는 다음행으로 이동한다.
    
    Close Cursor : 커서 닫기로 결과셋의 자원을 반납한다. select문장이 모두
    처리된 후 Cursor를 닫아준다.
    
    Cursor의 속성
        %Found : 가장 최근엔 인출(Fetch)이 행을 Return하면 True, 아니면 
        False를 반환한다.
...
begin
    ...
    */
    open cur1;
    -- basuc 루프믄으로 얻어온 결과셋의 갯수만큼 반복하여 인출한다.
    loop
        ...
        fetch cur1 into
            ...
            v_dept.location_id;
        -- 탈출 조건으로 더이상 출력할 행이 없으면 exit가 실행된다.
...
/*
컬렉션(배열)
일반 프로그래밍 언어에서 사용하는 배열타입을 PL/SQL에서는 테이블타입이라고 한다.
...
Key와 Value의 한 쌍으로 구성된 컬렉션으로 Java의 해시맵과 같은 개념이다.
...
declare
    ...
    Type avType Is
        ...
        Index By varchar2(10);  -- key(키 즉 인덱스)의 자료형 선언
    -- 연관배열 타입의 변수 선언
    var_array avType;
begin
    -- 연관배열 값 할당
...
-- 문제의 조건을 통한 쿼리에서 다수 행이 출력되었으므로 Cursor를 사용한다.
declare
    -- 쿼리문을 통해 커서를 생성한다.
    cursor emp_cur is
...
/*
고정길이를 가진 배열로써 일반프로그래밍 언어에서 사용하는 배열과 동일하다.
크기에 제한이 있어서 선언할때 크기(원소의개수)를 지정하면 이보다 큰 배열로
만들 수 없다.
...
declare
    -- VArray 자료형 선언. 배열에 저장된 값은 사원아이디 컬럼을 참조하여
    -- 생성한다.
    type vaType1 is
...
/*
중첩 테이블(Nested Table)
VArray와 비슷한 구조의 배열로써 배열의 크기를 명시하지 않으므로 종적으로
...
begin
    ...
    -- 에러 발생 . 첨자가 개수를 넘었습니다 ( 자동으로 확장되지 않는다. )
--  nt_array(5) := '다섯번째값??할당??';
