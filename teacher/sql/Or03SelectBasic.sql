...
형식]
    select 컬럼1, 컬럼2, ... 혹은 *
        ...
        order vy 정렬할컬럼 asc(오름차순), desc(내림차순);
*/
...
-> last_name이 Simth이면서 급여가 8000인 사원을 추출하시오.
*/
-- 컬럼이 문자형인경우 싱글쿼테이션으로 감싼다. 숫자인경우 생략한다.
...
*/
-- 방법1 : or 를 사용한다. 이때 컬럼명을 반복적으로 기술해야 하므로 불편하다.
...
distinct
    ...
    특정 조건으로 select했을때 하나의 컬럼에서 중보되는 값이 있는 겨우
...
-- first_name이 세번째문자가 a인 직원을 추출하시오.
...
레코드 종렬하기(sorting)
    ...
    내림차순 정렬 : oeder by 컬럼명 desc 
    
    2개 이상의 컬럼으로 정렬해야 할 경우 콤마로 구분해서 정렬한다.
    단, 이때 먼저 입력한 컬럼으로 정렬된 상태에서 두번재 컬럼이 정렬된다.
*/
/*
사원정보 테이블에서 급여가 낮은 순서에서 높은 순서로 인출되도록 정렬하여
조회하시오.
출력할 컬럼 : first_name, salary, email. phoun_number
*/
...
/*
부서번오를 내림차순으로 정렬한 후 해당 부서에서 낮은 급여를 받는 직원이 먼저
출력되도록 하는 SQL문을 작성하시오.
출력항목 : 사원번호, 이름, 서ㅇ, 급여, 부서번호
*/
