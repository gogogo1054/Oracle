...
group by
    ...
    - 최대, 최소, 평균, 합산 등의 연산이 가능하가.
*/
-- 각 담당업무별 직원수가 몇명인지 카운트 한다.
select job_id, count(*) from employees group by job_id;

-- 검증을 위해 해당 업무를 통해 select 해서 인출되는 행의 객수와 비교해본다.
...
* 쿼리의 실행 순서
    from(태이블) -> where(조건) -> group by(그룹화) -> having(그룹조건)
...
select 
    ...
    ltrim(to_char(sum(salary), 'L999,000')) "죄측공백제거",
    ltrim(to_char(sum(salary), '$999,000')) "통화기호 삽입"
 from employees where department_id =10; 

-- sum() 과 같은 그룹합수는 number타입인 컬럼에서만 사용할 수 있다.
select sum(first_name) from employees; --에러발생

/*
count() : 그룹화된 레코드의 객수를 카운트할때 사용하는 함수.
*/
...
select
    ...
    count(distinct job_id) " 순수담당 업무 객수"
 from employees;

/*
avg() : 평균값을 구할때 사용하는 함수
*/
-- 전체사원의 평규급여는 얼마인지 출력하는 쿼리문을 작성하시오.
select
    count(*) "전체사원수",
    sum(salary) "사원급여의 합",
...
-- 1. 부서테이블에서 영업팀의 부서번호가 무어인지 확인한다.
/*
...
-- 아래쿼리문은 에버발생됨. 그룹함수는 일반컬럼에 사용할 수 없다.
select first_name, salary from employees where salary=min(salary);

-- 사원테이블에서 가장 낮은 급여인 2100을 받는 사월을 인출한다.
...
/*
    ...
    다라서 문제에 따라 서브쿼리를 사용할지 여부를 결정해야한다.
*/
...
/*
step1 : 부서가 많은 경우 일일이 부사별로 확인할 수 없으므로 부서를
...
/*
시나리오] 부서아이디가 50인 사원들의 직원총합, 평균급여, 급요총합이
...
시나리오] 사원테이블에서 각 부서별로 근무하고 있는 직원의 담당업무별
    사원수와 평균급여가 얼마인지 출력하는 쿼리문을 작석하시오.
    단, 사원수가 10을 초과하는 레코드만 인출하시오.
*/
/*
같은 부서에 근무하더라도 담당업무는 다를수 있으므로 임 문제에서는 
...
/*
    ...
    where절에 쓰면 에러가 발생한다. 이런 겨웅에는 having절에 조건을
    추가해야 한다.
    Ex) 급여가 3000인 사워 => 물리적으로 존재하므로 where절 사용
